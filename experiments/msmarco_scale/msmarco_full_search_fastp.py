...
class FullIndex:
 ...
 def prepare(self,text,hmax=20,pool_max=P):
  ...
  want=min(len(tail),pool_max+hmax+8)
  if len(tail)>want:
 ...
 def rank_h(self,p,h,k=100,pool=P):
  if p is None:return []
  ...
  keep=np.asarray([int(d) not in fs for d in p['cand_docs']],bool); docs=p['cand_docs'][keep][:pool]; ts=p['cand_tail'][keep][:pool]; lx=p['lex'][keep][:pool]; sm=p['sem'][keep][:pool]
...
if __name__=='__main__':
 print('import this module from a sweep script')
