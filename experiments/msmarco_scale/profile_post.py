...
pr=cProfile.Profile(); pr.enable(); p=idx.prepare(txt,20); r=idx.rank_h(p,0,100); pr.disable(); s=io.StringIO(); pstats.Stats(pr,stream=s).sort_stats('cumtime').print_stats(30); print(s.getvalue())
