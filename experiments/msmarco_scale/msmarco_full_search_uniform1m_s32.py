...
ROOT=Path('/mnt/data'); WORK=ROOT/'msmarco_scale_work'; GEOM=WORK/'geometry_uniform1m_s32'; IDX=WORK/'full_index_uniform1m_s32_post'
...
class FullIndex:
 def __init__(self):
  ...
  self.dl=np.memmap(IDX/'doc_lengths.u16',np.uint16,'r',shape=(N,)); self.offs=np.load(IDX/'branch_offsets.npy',mmap_mode='r'); _np=int(self.offs[-1]); self.pd=np.memmap(IDX/'post_doc.u32',np.uint32,'r',shape=(_np,)); self.pm=np.memmap(IDX/'post_membership.f32',np.float32,'r',shape=(_np,)); self.pr=np.memmap(IDX/'post_res_terms.u16',np.uint16,'r',shape=(_np,S)); self.ps=np.memmap(IDX/'post_signbits.u32',np.uint32,'r',shape=(_np,))
...
if __name__=='__main__':
 ...
 out={'protocol':'h tuned on deterministic 1000-query TRAIN sample; full DEV untouched','best_h':best,'validation':{str(h):vv for h,vv in rows},'validation_timing':val_timing,'dev_metrics':mm,'timing':timing,'index_meta':idx.meta,'geometry_note':'full 8.84M vocabulary/IDF; uniform 1M centers/graph; residual width and reliability S=32 capacity test'}
 with open(WORK/'full_msmarco_uniform1m_s32_results.json','w') as f: json.dump(out,f,indent=2)
 print('DEV_METRICS',mm,flush=True); print('TIMING',timing,flush=True); print('saved',WORK/'full_msmarco_uniform1m_s32_results.json',flush=True)
