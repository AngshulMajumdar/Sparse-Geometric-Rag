...
def prepare(text):
 ...
 qmask=np.zeros(M,np.uint8); qmask[q.indices]=1; rarerank=np.zeros(M,np.uint8); ordq=q.indices[np.argsort(idx.idf[q.indices])[::-1]]
 for r,t in enumerate(ordq[:3],start=1):rarerank[t]=r
 lx,sm,qc,r3=final_features(dd,idx.sup_ip,idx.sup_ids,qmask,rarerank,idx.idf,semvec,idx.dl,idx.avgdl); cov=qc/max(1,len(q.indices)); ladj=lx*np.power(np.maximum(cov,1e-6),ALPHA); rarecov=r3/max(1,min(3,len(q.indices)))
 fin=m.zscore(ts)+WLEX*m.zscore(ladj)+WSEM*m.zscore(sm)+WRARE*m.zscore(rarecov); oo=np.argsort(fin)[::-1][:100]
 return [int(x) for x in dd[oo]],ud,sel
_=final_features(np.array([0],np.uint32),idx.sup_ip,idx.sup_ids,np.zeros(M,np.uint8),np.zeros(M,np.uint8),idx.idf,np.zeros(M,np.float32),idx.dl,idx.avgdl)
...
met=m.eval_run(run,qrels);result={'protocol':'IDF^2 + top-3 rare-query-term coverage selected on five disjoint 1000-query TRAIN folds; untouched full DEV','params':{'preselection_eta':1.0,'preselection_idf_power':1.0,'P':2000,'gamma_tail':0.25,'lambda_M':0.125,'final_idf_power':2.0,'final_length_b':0.1,'coordination_alpha':0.25,'lambda_lex':4.0,'lambda_sem':0.3,'rare_topk':3,'rare_coverage_weight':1.0,'h':0,'S':16},'dev_metrics':met,'route_relevant_recall':routehit/den,'pool_relevant_recall':poolhit/den,'timing':{'median_ms':float(np.median(times)),'p95_ms':float(np.percentile(times,95)),'mean_ms':float(np.mean(times)),'qps':1000/float(np.mean(times)),'avg_candidate_docs':float(np.mean(cands))}}
out=WORK/'idf2_rare3_best_dev_results.json';json.dump(result,open(out,'w'),indent=2);print('RESULT',json.dumps(result,indent=2),flush=True);print('SAVED',out,flush=True)
