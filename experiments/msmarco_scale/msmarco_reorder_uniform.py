...
import multiprocessing as mp,time,os,json
import numpy as np
WORK=Path('/mnt/data/msmarco_scale_work'); OLD=WORK/'full_index'; NEW=WORK/'full_index_uniform1m'
N=8_841_823; F=4; S=16
npost=int(np.load(OLD/'branch_offsets.npy',mmap_mode='r')[-1])

def job(a,b):
    t=time.time(); bo=np.memmap(OLD/'branch_order.u32',np.uint32,'r',shape=(npost,)); rt=np.memmap(NEW/'res_terms.u16',np.uint16,'r',shape=(N,F,S)); sb=np.memmap(NEW/'signbits.u16',np.uint16,'r',shape=(N,F)); pr=np.memmap(NEW/'post_res_terms.u16',np.uint16,'r+',shape=(npost,S)); ps=np.memmap(NEW/'post_signbits.u16',np.uint16,'r+',shape=(npost,))
    block=500_000
...
if __name__=='__main__':
    ...
    if not (NEW/'post_signbits.u16').exists(): np.memmap(NEW/'post_signbits.u16',np.uint16,'w+',shape=(npost,)).flush()
    # create symlinks to geometry-independent structures
    ...
    print('REORDER DONE npost',npost,'sec',time.time()-t,flush=True)
