...
ETA=np.float32(1.0); FINAL_B=np.float32(0.1); ALPHA=np.float32(0.25); WLEX=np.float32(4.0); WSEM=np.float32(0.3)
set_num_threads(5)
...
def selected_final_features_idf2(dd,ip,ids,qmask,idf,semvec,dl,avgdl):
 ...
 for z in prange(n):
  ...
  for k in range(a,bb):
   t=int(ids[k]); s+=semvec[t]
   if qmask[t]:
    x=float(idf[t]); raw+=x*x; c+=1.0
...
def prepare(text):
 ...
 # Early rescue remains the validated IDF^1 whole-document score with b=.2.
 ...
 lx,sm,qc=selected_final_features_idf2(dd,idx.sup_ip,idx.sup_ids,qmask,idx.idf,semvec,idx.dl,idx.avgdl)
...
_=selected_final_features_idf2(np.array([0],np.uint32),idx.sup_ip,idx.sup_ids,np.zeros(M,np.uint8),idx.idf,np.zeros(M,np.float32),idx.dl,idx.avgdl)
...
met=m.eval_run(run,qrels); result={'protocol':'IDF^2 final lexical rule selected robustly on five disjoint 1000-query TRAIN folds; untouched full DEV','params':{'preselection_eta':1.0,'preselection_lexical_idf_power':1.0,'preselection_length_b':0.2,'P':2000,'gamma_tail':0.25,'lambda_M':0.125,'final_lexical_idf_power':2.0,'final_length_b':0.1,'coordination_alpha':0.25,'lambda_lex':4.0,'lambda_sem':0.3,'h':0,'S':16},'dev_metrics':met,'route_relevant_recall':routehit/den,'pool_relevant_recall':poolhit/den,'timing':{'median_ms':float(np.median(times)),'p95_ms':float(np.percentile(times,95)),'mean_ms':float(np.mean(times)),'qps':1000/float(np.mean(times)),'avg_candidate_docs':float(np.mean(cands))}}
out=WORK/'idf2_best_dev_results.json'; json.dump(result,open(out,'w'),indent=2); print('RESULT',json.dumps(result,indent=2),flush=True); print('SAVED',out,flush=True)
