...
def prepare(text):
    ...
    oo=np.argsort(fin)[::-1][:1000]
    return [int(x) for x in dd[oo]],ud,sel

def eval_beir(run,qrels):
    metrics={'nDCG@10':[],'nDCG@10_expGain_diag':[],'MRR@10':[],'P@10':[],'R@10':[],'R@100':[],'R@1000':[],'Hit@10':[],'Hit@100':[],'Hit@1000':[]}
    for qid,qr in qrels.items():
        rank=run.get(qid,[]); pos={int(d) for d,r in qr.items() if float(r)>=2.0}; n=max(1,len(pos))
        h10=sum(d in pos for d in rank[:10]); h100=sum(d in pos for d in rank[:100]); h1000=sum(d in pos for d in rank[:1000])
        metrics['P@10'].append(h10/10); metrics['R@10'].append(h10/n); metrics['R@100'].append(h100/n); metrics['R@1000'].append(h1000/n); metrics['Hit@10'].append(float(h10>0)); metrics['Hit@100'].append(float(h100>0)); metrics['Hit@1000'].append(float(h1000>0))
        rr=0.
...
for z,qid in enumerate(ids):
    ...
    rank,ud,sel=out; run[qid]=rank; cands.append(len(ud)); rels=[int(d) for d,r in qrels[qid].items() if float(r)>=2.0]; den+=len(rels); pooldocs=set(map(int,ud[sel].tolist()))
    for d in rels:
...
out=WORK/'FINAL_TEST_BASELINE_COMPARE_results.json'; json.dump(result,open(out,'w'),indent=2); print('FINAL_RESULT',json.dumps(result,indent=2),flush=True); print('SAVED',out,flush=True)
